...
class CargoUtils {
	...
	public static function commitStoresForPage( Title $title, ParserOutput $parserOutput ) {
		...
		$dbw = wfGetDB( DB_MASTER );
		$tableSchemas = [];
		foreach ( $cargoStorage as $storeData ) {
			$tableName = $storeData['tableName'];
			$tableFieldValues = $storeData['tableFieldValues'];
			if ( !isset( $tableSchemas[$tableName] ) ) {
				$tableSchemaData = $dbw->selectField(
					'cargo_tables',
					'table_schema',
					[ 'main_table' => $tableName ]
				);
				$tableSchemas[$tableName] = $tableSchemaData
					? CargoTableSchema::newFromDBString( $tableSchemaData )
					: false;
			}
			$tableSchema = $tableSchemas[$tableName];
			if ( !$tableSchema ) {
				wfDebugLog( 'cargo', "Skipping Cargo store on page save: table ($tableName) does not exist.\n" );
				continue;
			}
			CargoStore::storeAllData( $title, $tableName, $tableFieldValues, $tableSchema );
		}
		$cdb->commit();
	...
	public static function createTable( $cdb, $tableName, $fieldsInTable, $multipleColumnIndex = false ) {
		global $wgCargoDBRowFormat, $wgCargoDBCharset, $wgCargoDBCollation;
		...
		if ( !empty( $wgCargoDBCharset ) ) {
			$createSQL .= " DEFAULT CHARSET=$wgCargoDBCharset";
		}
		if ( !empty( $wgCargoDBCollation ) ) {
			$createSQL .= " COLLATE $wgCargoDBCollation";
		}
		$cdb->query( $createSQL );
